...
popover.magicBox = function(){
	...
	function copyMagic(){
		...
		var $box = $('<div>').css({position:'relative'});
		var magicData = [];
		$box.css({height:copyRows.length*_w});
		for(var i=0;i<copyRows.length;i++){
			for(var k=1;k<5;k++){
				var copyForm = $('#magicTable').find('.item[data-pos='+(copyRows[i]+''+k)+']');
				var magicDataItem = {isDelete:false};
				...
				var $item = $('<div>').css({width:_w*colspan,height:_w*rowspan,top:top,left:left,position:'absolute'});
				if(copyForm.is('.selected')){
					var $img = $('<img>').attr('src',copyForm.attr('data-img')).css({width:'100%',height:'100%'});
					$item.append($img);
				}
				$box.append($item);
			}
	...
	function createTd(k,j){
		var td = $('<div>',{class:'item','data-pos':k.toString()+j});
		var top = (k-1)*50 +k;
		var left = (j-1)*50+j;
	...
	$('#split-btn').on('click',function(){
		...
		if($merge.length > 0){
			...
			deleteArry.forEach(function(val){
				...
				$preItem.after($('<div>').css({top:t,left:l}).addClass('item').attr('data-pos',val));
			});
	...
	$('#magicBox-img').on('click',function(){
  	...
  	popover.d = dialog({
  		title:'选择图片',
  		content:$('#selectImg').tmpl({list:[{src:'images/xxximg.jpg'},{src:'images/xxximg01.jpg'},{src:'images/xxximg02.jpg'},{src:'images/xxximg03.jpg'}]})
  	});
  	popover.d.show();
	});//end magicImage
